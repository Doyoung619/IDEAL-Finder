...
def main() -> None:
    ...
    if args.candidate_count < 16:
        parser.error("--candidate-count must be at least 16")
    ...
    for offset, gender in enumerate(genders):
        ...
        print(f"Preparing {gender} startup candidates (seed={seed})...")
        ...
        print(f"Saved {len(candidates)} candidates to {Path(path)}")
