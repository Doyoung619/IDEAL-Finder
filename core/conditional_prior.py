...
class DemographicCondition:
    """Operational gender and race targets used to build a conditional prior."""
    ...
    def __post_init__(self) -> None:
        ...
        if not self.race_targets:
            raise ValueError("race_targets must contain at least one label")
...
class ConditionalPCAPrior:
    ...
    def transform_matrix(self) -> np.ndarray:
        ...
        return self.components.T * np.sqrt(self.eigenvalues)[None, :]
    ...
    def w_to_theta(self, w: np.ndarray) -> np.ndarray:
        ...
        result = (centered @ self.components.T) / np.sqrt(
            self.eigenvalues
        )[None, :]
        result = result.astype(np.float32)
        return result[0] if squeezed else result
